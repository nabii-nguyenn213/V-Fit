...
def main():
    ...
    local_security_config = r"d:\EXE_PRM\VFIT_Backend\src\main\java\com\vfit\security\config\SecurityConfig.java"
    remote_security_config = r"C:\V-Fit\VFIT_Backend\src\main\java\com\vfit\security\config\SecurityConfig.java"
    
    if not os.path.exists(local_security_config):
        print(f"[ERROR] Local file not found: {local_security_config}")
        sys.exit(1)
    ...
    try:
        ...
        # 1. Upload updated SecurityConfig.java
        print(f"[*] Opening SFTP connection to upload SecurityConfig.java...")
        sftp = ssh.open_sftp()
        sftp.put(local_security_config, remote_security_config)
        sftp.close()
        print("[+] Uploaded SecurityConfig.java successfully!")
        ...
        print("[*] Waiting for Spring Boot application to boot up and serve verification file...")
        
        max_attempts = 30
        poll_interval = 3
        success = False
        
        cmd_test = "curl.exe -s http://127.0.0.1:8080/googlec5a363f8efe4656c.html"
        
        for attempt in range(1, max_attempts + 1):
            print(f"[*] Attempt {attempt}/{max_attempts}: Checking verification endpoint...")
            status, out, err = execute_remote_cmd(ssh, cmd_test)
            if status == 0 and "google-site-verification" in out:
                print(f"[+] Local verification response: '{out.strip()}'")
                print("[+] SUCCESS! Verification file is served locally by Spring Boot without authentication.")
                success = True
                break
            else:
                print(f"[*] App not ready yet (Exit code: {status}). Waiting {poll_interval} seconds...")
                time.sleep(poll_interval)
                
        if not success:
            print("[ERROR] Verification test failed after waiting 90 seconds!")
